...
def plot_data(X_train, y_train, X_test, y_test, porcentagem):
  ...
  plt.subplot(131)
  plt.axis('equal')
  plt.scatter(X[:, 0], X[:, 1], c = colors, s = 5, edgecolors=colors)
  plt.title("Data (100%)")
  ...
  plt.axis('equal')
  #plt.axis('off')
  plt.scatter(X_train.iloc[:, 0].values, X_train.iloc[:, 1].values, c = colors_train, s = 5 , edgecolors=colors_train)
  plt.title("Training Data ({}%)".format(1-porcentagem))

  # testing data plot
  plt.subplot(133)
  plt.axis('equal')
  #plt.axis('off')
  plt.scatter(X_test.iloc[:, 0].values, X_test.iloc[:, 1].values, c = colors_test, s = 5, edgecolors=colors_test)
  plt.title("Test Data ({}%)".format(porcentagem))
  plt.tight_layout()
...
def plot_decision_function_helper(X, y, clf, show_only_decision_function = False):
  ...
  plt.scatter(X.iloc[:, 0].values, X.iloc[:, 1].values, c = colors, s = 10, edgecolors=colors)
  ax = plt.gca()
  xlim = ax.get_xlim()
  ylim = ax.get_ylim()
